...
DESCRIPTION:
        This is a sequential version of the TP3
             Communication locale / globale - Différences finies appliquées à 
             l’équation de la chaleur bidimensionnelle

INSTRUCTIONS:
        How to compile: mpicc –o sequential_heat2D sequential_heat2D.c -lm
        How to execute: mpirun -np NB_Of_THREADS sequential_heat2D
...
int main(int argc, char **argv)
{
	...
	/* Calcul de la diffusion de la chaleur */
        ...
        for (i=0; i<nbLigs; i++){
            for (j=0; j<nbCols; j++){
                printf("%6.1f%c", *(data2+i*nbCols+j),(j==nbCols-1)?'\n':' ');
            }
        }
